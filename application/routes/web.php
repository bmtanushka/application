...
Route::group(['prefix' => 'invoices'], function () {
    ...
    Route::post("/{invoice}/mark-sent", "Invoices@markSent")->where('invoice', '[0-9]+');
...
Route::group(['prefix' => 'tasks'], function () {
    ...
    Route::post("/{file}/file-comment", "Tasks@storeFileComment")->where('file', '[0-9]+');
    ...
    Route::post("/{task}/update-job-number", "Tasks@updateJobNumber")->where('task', '[0-9]+');
...
Route::get('/test-digest-mail', function() {
    try {
        // Get a client
        $client = \App\Models\User::where('type', 'client')->first();
        
        if (!$client) {
            return 'No client found! Create a client user first.';
        }
        
        // Create fake grouped notifications
        $groupedNotifications = [
            'tasks' => [
                [
                    'subject' => 'Test Task Assignment',
                    'message' => '<p>You have been assigned to Task #123</p>',
                    'created_at' => now(),
                    'resource_type' => 'task',
                    'resource_id' => 1,
                ]
        ...
        // Create fake digest emails collection
        $digestEmails = collect([
            (object)[
                'emailqueue_subject' => 'Test',
                'emailqueue_message' => 'Test message',
            ]
        ]);
        
        // Try to create the mail
        $mail = new \App\Mail\ClientDailyDigest($client, $groupedNotifications, $digestEmails);
        
        // Try to render it (this will show errors)
        $rendered = $mail->render();
        
        return 'Mail rendered successfully! Length: ' . strlen($rendered) . ' characters. Check if it looks good:<hr>' . $rendered;
        
    } catch (\Exception $e) {
        return '<h3>Error:</h3><pre>' . $e->getMessage() . "\n\n" . $e->getTraceAsString() . '</pre>';
    }
});


Route::get('/send-digest-now', function() {
    try {
        // Get a client - replace with your test email
        $client = \App\Models\User::where('email', 'biz_lakmi@yahoo.com')->first();
        
        if (!$client) {
            return 'Client not found! Email: biz_lakmi@yahoo.com';
        }
        ...
        $groupedNotifications = [
            'tasks' => [
                [
                    'subject' => 'TEST: Task Assignment',
                    'message' => '<p>This is a test digest email from the route.</p>',
                    'created_at' => now(),
                    'resource_type' => 'task',
                    'resource_id' => 1,
                ]
        ...
        $digestEmails = collect([
            (object)[
                'emailqueue_subject' => 'Test',
                'emailqueue_message' => 'Test',
            ]
        ]);
        
        // Send the email
        \Mail::to($client->email)->send(new \App\Mail\ClientDailyDigest($client, $groupedNotifications, $digestEmails));
        
        return 'Email sent to ' . $client->email . '! Check your inbox (and spam).';
...
// Add this route
Route::get('/check-mail-driver', function() {
    return [
        'driver' => config('mail.driver'),
        'queue' => config('mail.queue'),
        'host' => config('mail.host'),
        'from' => config('mail.from'),
    ];
});

Route::get('/send-real-digest', function() {
    try {
        // Get the client
        $client = \App\Models\User::where('email', 'biz_lakmi@yahoo.com')->first();
        ...
        // Get real grouped notifications from queue
        $digestEmails = \App\Models\EmailQueue::where('emailqueue_delivery_type', 'digest')
            ->where('emailqueue_to', 'biz_lakmi@yahoo.com')
            ->where('emailqueue_status', 'new')
            ->get();
        
        if ($digestEmails->isEmpty()) {
            return 'No digest emails in queue for this user. Create one first.';
        }
        ...
        foreach ($digestEmails as $email) {
            $category = 'other';
            if (stripos($email->emailqueue_subject, 'task') !== false) {
                $category = 'tasks';
            } elseif (stripos($email->emailqueue_subject, 'project') !== false) {
                $category = 'projects';
            }
            
            $grouped[$category][] = [
                'subject' => $email->emailqueue_subject,
                'message' => $email->emailqueue_message,
        ...
        $output = '<h2>Sending Real Digest</h2>';
        $output .= '<p>To: ' . $client->email . '</p>';
        $output .= '<p>Notifications: ' . $digestEmails->count() . '</p>';
        
        // Create and send
        $mail = new \App\Mail\ClientDailyDigest($client, $grouped, $digestEmails);
        
        // Try to render first
        $rendered = $mail->render();
        $output .= '<p>Rendered size: ' . strlen($rendered) . ' bytes</p>';
        
        // Send it
        \Mail::to($client->email)->send($mail);
        
        $output .= '<p><strong>✅ Email sent! Check inbox.</strong></p>';
        $output .= '<hr><h3>Preview:</h3>' . $rendered;
...
Route::get('/compare-emails', function() {
    $client = \App\Models\User::where('email', 'biz_lakmi@yahoo.com')->first();
    ...
    $output = '<h1>Email Comparison</h1>';
    
    // Working email (simple test)
    $output .= '<h2>Working Email (Test)</h2>';
    $output .= '<p>From: ' . config('mail.from.address') . '</p>';
    $output .= '<p>To: ' . $client->email . '</p>';
    $output .= '<p>Size: ~7KB</p>';
    $output .= '<p>Status: ✅ Arrives successfully</p>';
    
    // Digest email
    $output .= '<h2>Digest Email</h2>';
    try {
        $grouped = ['tasks' => [
            [
                'subject' => 'Test Task',
        ...
        $digestEmails = collect([(object)['emailqueue_subject' => 'Test']]);
        ...
        $output .= '<p>From: ' . config('mail.from.address') . '</p>';
        $output .= '<p>To: ' . $client->email . '</p>';
        $output .= '<p>Size: ' . strlen($rendered) . ' bytes (' . round(strlen($rendered)/1024, 2) . ' KB)</p>';
        
        // Check for potential issues
        if (strlen($rendered) > 102400) {
            $output .= '<p>⚠️ Email is larger than 100KB - might be rejected!</p>';
        }
        
        if (substr_count($rendered, '<img') > 10) {
            $output .= '<p>⚠️ Many images - might trigger spam filter</p>';
        }
        
        $output .= '<p>Status: ❌ Does not arrive</p>';
        
        $output .= '<hr><h3>Digest Preview (First 5000 chars):</h3>';
        $output .= '<textarea style="width:100%;height:300px;">' . htmlspecialchars(substr($rendered, 0, 5000)) . '</textarea>';
...
Route::get('/force-clear-all', function() {
    ...
    try {
        ...
        // Show current config after clearing
        $output .= '<hr><h3>Current Mail Config (after clearing):</h3>';
        $output .= '<pre>';
        $output .= 'Driver: ' . config('mail.driver') . "\n";
        $output .= 'Host: ' . config('mail.host') . "\n";
        $output .= 'Port: ' . config('mail.port') . "\n";
        $output .= 'From Address: ' . config('mail.from.address') . "\n";
        $output .= 'From Name: ' . config('mail.from.name') . "\n";
        $output .= '</pre>';
        
        $output .= '<p><strong>Now test the digest again!</strong></p>';
