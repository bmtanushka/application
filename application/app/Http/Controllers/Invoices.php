...
use App\Http\Responses\Invoices\MarkSentResponse;
...
class Invoices extends Controller {
    ...
    public function attachProject() {
        ...
        //];
        $payload = [
            'projects_feed_url' => url("/feed/tasks?ref=clients_projects&client_id=$client_id"),
        ];
    ...
    /**
     * update bill_sent field
     * @return \Illuminate\Http\Response
     */
    public function markSent($bill_invoiceid = '') {
        ...
        if($invoice->bill_sent)
            $invoice->bill_sent = null;
        else
            $invoice->bill_sent = 1;
        $invoice->save();

        //show the form
        return new MarkSentResponse();
    }
