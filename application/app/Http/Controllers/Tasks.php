...
use App\Http\Responses\Tasks\StoreFileCommentResponse;
...
class Tasks extends Controller {
    ...
    public function store(TaskStoreUpdate $request, TaskAssignedRepository $assignedrepo) {
        ...
        /*
        //===================================================================
        // MODIFIED: Immediately open full task view after creation
        // Solution: Just call the existing show() method!
        //===================================================================
        
        // Initialize repositories required by show() method
        // Resolve repositories with dependencies using app() helper
        
        $commentrepo = app(\App\Repositories\CommentRepository::class);
        $attachmentrepo = app(\App\Repositories\AttachmentRepository::class);
        $checklistrepo = app(\App\Repositories\ChecklistRepository::class);
        $projectassignedrepo = app(\App\Repositories\ProjectAssignedRepository::class);
        
        // Call show() to open full task interface immediately
        return $this->show(
            $assignedrepo,
            $projectassignedrepo,
            $commentrepo,
        ...
        //===================================================================
        // END OF MODIFICATION - Only 13 lines! Much better!
        //===================================================================
        
        // OLD CODE (commented out - this was the original):
        //return new StoreResponse($payload);
        */
    }
    ...
    /**
     * update task job number
     * @param int $id task id
     * @return \Illuminate\Http\Response
     */
    public function updateJobNumber($id) {

        //get the task
        $task = $this->taskmodel::find($id);

        //validation
        if (hasHTML(request('task_job_number'))) {
            //[type options] error|success
            $jsondata['notification'] = [
                'type' => 'error',
                'value' => __('lang.title') . ' ' . __('lang.must_not_contain_any_html'),
            ...
            //update back the title
            $jsondata['dom_html'][] = [
                'selector' => '#card-job-number-editable',
                'action' => 'replace',
                'value' => safestr($task->task_job_number),
            ];
            return response()->json($jsondata);
        }

        //validation
        if (!request()->filled('task_job_number')) {
            ...
            $task->task_job_number = request('task_job_number');
            $task->save();
    ...
    /**
     * save task file comment
     * @param object CommentRepository instance of the repository
     * @return \Illuminate\Http\Response
     */
    public function storeFileComment(CommentRepository $commentrepo, AttachmentRepository $attachmentrepo, $id) {
        ...
        request()->merge([
            'commentresource_type' => 'attachment',
            'commentresource_id' => $id,
            'comment_text' => request('comment_text'),
        ]);
        $comment_id = $commentrepo->create();
        ...
        $attachments = $attachmentrepo->search($id);
        $attachment = $attachments->first();
        //get task
        $tasks = $this->taskrepo->search($attachment->attachmentresource_id);
        $task = $tasks->first();
        $this->processTask($task);
        ...
        //show the form
        return new StoreFileCommentResponse($payload);
    }
    ...
    public function attachFiles(Request $request, AttachmentRepository $attachmentrepo, $id) {
        ...
        if ($file = $request->file('file')) {
            ...
            $data = [
                ...
                'attachmentresource_type' => 'task',
                'attachmentresource_id' => $id,
                'attachment_completed' => $request['completed'],
            ];
