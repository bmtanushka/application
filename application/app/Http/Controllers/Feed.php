...
class Feed extends Controller {
    ...
    /**
     * ajax search results for tasks
     * calls to this function MUST have a request('ref'), so that a specific check in done
     * @permissions team members only
     * @return \Illuminate\Http\Response
     */
    public function tasks(TaskRepository $taskrepo) {
        ...
        if (request('ref') == 'general' && auth()->user()->role->role_projects > 0) {
            if (auth()->user()->role->role_projects_scope == 'global') {
                $feed = $taskrepo->autocompleteFeed('', '', request('term'));
            } else {
                $feed = $taskrepo->autocompleteFeed('', 'assigned', request('term'));
            }
        }
        ...
        if (request('ref') == 'clients_projects') {
            if (auth()->user()->role->role_projects_scope == 'global') {
                $feed = $taskrepo->autocompleteClientsTasksFeed('', '', request('client_id'), request('term'));
            } else {
                $feed = $taskrepo->autocompleteClientsTasksFeed('', 'assigned', request('client_id'), request('term'));
            }
        }
