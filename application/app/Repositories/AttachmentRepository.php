...
class AttachmentRepository {
    ...
    public function search($id = '') {
        ...
        $attachments->select( 'attachments.*','users.*', 
            DB::raw('GROUP_CONCAT(CONCAT(reactions.reaction, ",", reactions.name) SEPARATOR ";") as emoji_data'),
            DB::raw('GROUP_CONCAT(CONCAT(comments.comment_id, "~|~", comments.comment_text, "~|~", p.comment_text, "~|~", u.first_name, "~|~", comments.commentparent_id, "~|~", comments.commentsuperparent_id, "~|~", comments.comment_created, "~|~", u.id) SEPARATOR ";|;") as comments_data') );
            
        $attachments->groupBy('attachments.attachment_id');
        ...
        $attachments->leftJoin('reactions', function($join) {
            $join->on('reactions.reaction_resource_id', '=', 'attachments.attachment_id')
             ->where('reaction_resource_type', '=', 'attachment');
        });
        
        $attachments->leftJoin('comments', function($join) {
            $join->on('comments.commentresource_id', '=', 'attachments.attachment_id')
             ->where('comments.commentresource_type', '=', 'attachment');
        });
        
        $attachments->leftJoin('comments as p', 'comments.commentparent_id', '=', 'p.comment_id');
        
        $attachments->leftJoin('users as u', 'comments.comment_creatorid', '=', 'u.id');
        ...
        //default sorting
        $attachments->orderBy('attachment_id', 'desc')->orderBy('comments.comment_created', 'desc');
    ...
    public function create($data = '') {
        ...
        if(isset($data['attachment_completed']) && $data['attachment_completed'] == 'true')
            $attachment->attachment_completed = 1;
        //save and return id
        if ($attachment->save()) {
