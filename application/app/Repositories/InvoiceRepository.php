...
class InvoiceRepository {
    ...
    public function search($id = '', $data = array()) {
        ...
        $invoices->leftJoin('tasks', 'tasks.task_id', '=', 'invoices.bill_taskid');
        ...
        //filter: invoice (mtd)
        if (isset($_GET['db_filter_bill_due_date_mtd']) && $_GET['db_filter_bill_due_date_mtd'] == 'yes') {
            $invoices->whereDate('bill_due_date', '>=', date("Y-m-01", strtotime(date("Y-m-d"))));
            $invoices->whereDate('bill_due_date', '<=', date("Y-m-d", strtotime('last day of this month')));
            if(isset($_GET['db_project_status']) && $_GET['db_project_status'] == 'in-progress') {
                $invoices->where('task_status','<>', '2');
            }
            if(isset($_GET['db_project_status']) && $_GET['db_project_status'] == 'completed') {
                $invoices->where('task_status', '2');
            }
        }
        
        if (isset($_GET['filter_bill_due_date_type']) && $_GET['filter_bill_due_date_type'] == "custom") {
            if (isset($_GET['filter_bill_due_date_start_client']) && $_GET['filter_bill_due_date_start_client'] != "") {
                $dateTokens = explode('-', $_GET['filter_bill_due_date_start_client']);
                $client_start_date = $dateTokens[2]."-".$dateTokens[0]."-".$dateTokens[1];
                $invoices->whereDate('bill_due_date', '>=', $client_start_date);
            }
            
            if (isset($_GET['filter_bill_due_date_end_client']) && $_GET['filter_bill_due_date_end_client'] != "") {
                $dateTokens = explode('-', $_GET['filter_bill_due_date_end_client']);
                $client_end_date = $dateTokens[2]."-".$dateTokens[0]."-".$dateTokens[1];
                $invoices->whereDate('bill_due_date', '<=', $client_end_date);
            }
        } else if(isset($_GET['filter_bill_due_date_type']) && $_GET['filter_bill_due_date_type'] == "this-month") {
            $invoices->whereDate('bill_due_date', '>=', date("Y-m-d", strtotime('first day of this month')));
            $invoices->whereDate('bill_due_date', '<=', date("Y-m-d", strtotime('last day of this month')));
        } else if(isset($_GET['filter_bill_due_date_type']) && $_GET['filter_bill_due_date_type'] == "last-month") {
            $invoices->whereDate('bill_due_date', '>=', date("Y-m-d", strtotime('first day of previous month')));
            $invoices->whereDate('bill_due_date', '<=', date("Y-m-d", strtotime('last day of previous month')));
        } else if(isset($_GET['filter_bill_due_date_type']) && $_GET['filter_bill_due_date_type'] == "this-year") {
            $invoices->whereDate('bill_due_date', '>=', date("Y-m-d", strtotime('first day of january this year')));
            $invoices->whereDate('bill_due_date', '<=', date("Y-m-d", strtotime('last day of december this year')));
        } else if(isset($_GET['filter_bill_due_date_type']) && $_GET['filter_bill_due_date_type'] == "last-year") {
            $invoices->whereDate('bill_due_date', '>=', date("Y-m-d", strtotime('first day of january previous year')));
            $invoices->whereDate('bill_due_date', '<=', date("Y-m-d", strtotime('last day of december previous year')));
        }
    ...
    public function create() {
        ...
        //if(request('bill_clientid')==9){
        if(in_array(request('bill_clientid'), SP_CLIENTS)){
            $invoice->bill_taskid = request('bill_projectid');
            $invoice->bill_projectid = 169;
        } else {
