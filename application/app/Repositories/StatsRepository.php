...
class StatsRepository {
    ...
    public function sumCountInvoices($data = []) {
        ...
        if (isset($data['client_id']) && is_numeric($data['client_id']) && in_array($data['client_id'], SP_CLIENTS) ) {
            $invoices->leftJoin('tasks', 'tasks.task_id', '=', 'invoices.bill_taskid');
            if (isset($data['status']) && $data['status'] == 'due') {
                ...
                $invoices->Where('bill_status','<>', 'draft');
                $data['status'] = '';
            }
            if (isset($data['status']) && $data['status'] == 'overdue') {
                ...
                $invoices->where('task_status','<>', '2');
                $invoices->Where('bill_status','<>', 'draft');
                //$invoices->orWhere('bill_status','<>', 'overdue');
                $data['status'] = '';
            }
        }
