...
class ProjectRepository {
     ...
     * Inject dependecies
     */
    public function __construct(Project $projects, Task $tasks) {
    ...
    public function search($id = '', $data = []) {
        ...
        $projects->selectRaw("(SELECT event_id
                                      FROM events
                                      WHERE eventresource_id = projects.project_id
        ...
        $projects->selectRaw("(SELECT COUNT(*)
                                      FROM comments 
                                      WHERE commentresource_id = projects.project_id and commentresource_type = 'project')
                                      AS count_all_comments");
        ...
        if ($data['apply_filters']) {
            ...
            //echo $_GET['db_project_status'];
            //$projects->where('project_status', 'completedx');
    ...
    public function autocompleteClientsProjectsFeed($status = '', $limit = '', $client_id = '', $searchterm = '') {
        ...
        //start
        if(is_numeric($client_id) && $client_id==9){
            $query = $this->tasks->newQuery();
            $query->selectRaw("task_title AS value, task_id AS id");
            ...
            if (is_numeric($client_id)) {
                $query->where('task_clientid', '=', $client_id);
            }
