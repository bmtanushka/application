<?php

/** --------------------------------------------------------------------------------
 * This classes renders the [task comment] email and stores it in the queue
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/

namespace App\Mail;

use Illuminate\Bus\Queueable;
use Illuminate\Mail\Mailable;

class TaskComment extends Mailable {
    use Queueable;

    /**
     * The data for merging into the email
     */
    public $data;

    /**
     * Model instance
     */
    public $obj;

    /**
     * Model instance
     */
    public $user;

    public $emailerrepo;

    /**
     * Create a new message instance.
     *
     * @return void
     */
    public function __construct($user = [], $data = [], $obj = []) {

        $this->data = $data;
        $this->user = $user;
        $this->obj = $obj;

    }

    /**
     * Build the message.
     *
     * @return $this
     */
    public function build() {

        //email template
        if (!$template = \App\Models\EmailTemplate::Where('emailtemplate_name', 'Task Comment')->first()) {
            return false;
        }

        //validate
        if (!$this->obj instanceof \App\Models\Task || !$this->user instanceof \App\Models\User) {
            return false;
        }

        //only active templates
        if ($template->emailtemplate_status != 'enabled') {
            return false;
        }

        //check if clients emails are disabled
        if ($this->user->type == 'client' && config('system.settings_clients_disable_email_delivery') == 'enabled') {
            return;
        }

        //get the task status
        if ($task_status = \App\Models\TaskStatus::Where('taskstatus_id', $this->obj->task_status)->first()) {
            $status = $task_status->taskstatus_title;
        } else {
            $status = '---';
        }

        //get the task status
        if ($task_status = \App\Models\TaskStatus::Where('taskstatus_id', $this->obj->task_status)->first()) {
            $status = $task_status->taskstatus_title;
        } else {
            $status = '---';
        }

        //get common email variables
        $payload = config('mail.data');
        
        //FIX: Convert relative image URLs to absolute URLs
        $comment_text = $this->data['comment_text'];
        $comment_text = $this->fixImageUrls($comment_text);


        //set template variables
        $payload += [
            'first_name' => $this->user->first_name,
            'last_name' => $this->user->last_name,
            //'comment' => $this->data['comment_text'],
            'comment' => $comment_text,
            'poster_first_name' => $this->data['first_name'],
            'poster_last_name' => $this->data['last_name'],
            'task_id' => $this->obj->task_id,
            'task_title' => $this->obj->task_title,
            'task_created_date' => runtimeDate($this->obj->task_created),
            'task_date_start' => runtimeDate($this->obj->task_date_start),
            'task_description' => $this->obj->task_description,
            'task_date_due' => runtimeDate($this->obj->task_date_due),
            'project_title' => $this->obj->project_title,
            'project_id' => $this->obj->project_id,
            'client_name' => $this->obj->client_company_name,
            'client_id' => $this->obj->task_clientid,
            'task_status' => $status,
            'task_milestone' => $this->obj->milestone_title,
            'task_url' => url('/tasks/v/' . $this->obj->task_id . '/view'),
        ];

        //save in the database queue
        $queue = new \App\Models\EmailQueue();
        $queue->emailqueue_to = $this->user->email;
        $queue->emailqueue_subject = $template->parse('subject', $payload);
        $queue->emailqueue_message = $template->parse('body', $payload);
        $queue->emailqueue_resourcetype = 'project';
        $queue->emailqueue_resourceid = $this->obj->project_id;
        // NEW CODE: Determine delivery type based on user type and settings
        if ($this->user->type == 'client' && config('system.settings_clients_daily_digest') == 'enabled') {
            $queue->emailqueue_delivery_type = 'digest';
        } else {
            $queue->emailqueue_delivery_type = 'immediate';
        }
        $queue->save();
    }
    
    /**
     * Convert relative image URLs to absolute URLs for emails
     */
    private function fixImageUrls($html) {
        if (empty($html)) {
            return $html;
        }

        $base_url = url('/');

        // Fix <img src="/path"> to <img src="https://domain.com/path">
        $html = preg_replace_callback(
            '/<img([^>]*?)src=["\'](?!https?:\/\/)([^"\']*)["\']([^>]*?)>/i',
            function ($matches) use ($base_url) {
                $src = ltrim($matches[2], '/');
                $absolute_url = $base_url . '/' . $src;
                return '<img' . $matches[1] . 'src="' . $absolute_url . '"' . $matches[3] . '>';
            },
            $html
        );

        return $html;
    }
}
