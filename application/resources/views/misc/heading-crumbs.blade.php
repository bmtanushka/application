...
<div class="col-md-12 {{ runtimeCrumbsColumnSize($page['crumbs_col_size'] ?? '') }} align-self-center {{ $page['crumbs_special_class'] ?? '' }}" id="breadcrumbs">
    @if(in_array(auth()->user()['id'], SP_CLIENT_USERS) && $page['heading']=='Tasks')
    ...
    <ol class="breadcrumb">
        ...
        @if(in_array(auth()->user()['id'], SP_CLIENT_USERS) && $title=='Tasks')
            <li class="breadcrumb-item @if ($loop->last) active active-bread-crumb @endif">Jobs</li>
        @else
