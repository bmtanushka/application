...
<tr id="invoice_{{ $invoice->bill_invoiceid  }}">
    ...
    @endif
    @if(config('visibility.invoices_col_project') && auth()->user()['id'] != 22)
    ...
    @endif
    @if(auth()->user()->is_team || in_array(auth()->user()['id'], SP_CLIENT_USERS))
    <td class="invoices_col_task" id="invoices_col_task_{{ $invoice->bill_invoiceid }}">
        <a href="/tasks/v/{{ $invoice->bill_taskid }}" target="_new">{{ str_limit($invoice->task_title ?? '---', 12) }}</a>
    </td>
    @endif
    ...
    <td class="invoices_col_action actions_column" id="invoices_col_action_{{ $invoice->bill_invoiceid }}">
        ...
        <span class="list-table-action dropdown font-size-inherit">
            ...
            @if(!$invoice->bill_sent)
            <button type="button" title="Mark Sent"
                class="data-toggle-action-tooltip btn btn-outline-danger btn-circle btn-sm confirm-action-danger"
                data-confirm-title="Mark Sent"
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="POST"
                data-url="{{ url('/') }}/invoices/{{ $invoice->bill_invoiceid }}/mark-sent">
                <i class="sl-icon-check"></i>
            </button>
            @else
            <button type="button" title="Mark Unsent"
                class="data-toggle-action-tooltip btn btn-outline-success btn-circle btn-sm confirm-action-danger"
                data-confirm-title="Mark Unsent"
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="POST"
                data-url="{{ url('/') }}/invoices/{{ $invoice->bill_invoiceid }}/mark-sent">
                <i class="sl-icon-check"></i>
            </button>
