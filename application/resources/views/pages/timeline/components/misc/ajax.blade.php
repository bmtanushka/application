...
<div class="sl-item timeline">
        @if($variable != date("m-d-Y",strtotime($event->event_created)))
            <div class="sl-middle">
            {{ $variable = date("m-d-Y",strtotime($event->event_created)) }}  
            <br/>
            </div>
        @endif
    <div class="sl-left">
        <img src="{{ getUsersAvatar($event->avatar_directory, $event->avatar_filename, $event->event_creatorid)  }}" alt="user" class="img-circle" />
    </div>
    <div class="sl-right">
        <div>
            <div class="x-meta"><a href="javascript:void(0)" class="link">
                @if($event->event_creatorid == 0 || $event->event_creatorid == -1)
                    ...
                    @endif
                @else
                    {{ $event->first_name ?? runtimeUnkownUser() }} {{ $event->last_name ?? runtimeUnkownUser() }}
                @endif
            </a> <span class="sl-date"> at <!-- {{ runtimeDateAgo($event->event_created) }} --> {{date("H:i",strtotime($event->event_created))}} </span>
            </div>
            <div class="x-title">
