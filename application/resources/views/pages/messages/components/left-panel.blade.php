<div class="chat-left-aside menu-closed">
    ...
    <div class="chat-left-inner" id="chat-left-inner">
        <ul class="chatonline style-none" id="messages-left-menu">
            ...
            @if(auth()->user()->id==1 || auth()->user()->id==14 || auth()->user()->id==15 || auth()->user()->id==16 || auth()->user()->id==2 || auth()->user()->id==9)
            <!-- team 2 -->
            <li id="{{ messagesUniqueID(auth()->user()->unique_id, 'team2') }}">
                <a href="javascript:void(0)" class="ajax-request messages-menu-link" data-type="form"
                    id="messages_team2_link"
                    data-form-id="{{ messagesUniqueID(auth()->user()->unique_id, 'team2') }}"
                    data-ajax-type="post" data-loading-target="chat-messages-container"
                    data-message-target="team2" data-progress-bar="hidden"
                    data-counter-id="messages_counter_team" data-url="{{ url('/messages/feed?action=load') }}">
                    <img src="{{ url('public/images/team.png') }}" alt="user-img" class="img-circle">
                    <span>Internal Team
                        <small class="text-info messages_user_status text-lc">Brian, David, Mariana, Sebastian and Jim
                            <!--counter-->
                            <span class="badge badge-pill badge-danger messages_counter hidden"
                                id="messages_counter_team2">
                                <!--dynamic--></span>
                        </small>
                    </span>
                </a>
                <!--(dynamic)-->
                <input type="hidden" name="message_source" value="{{ auth()->user()->unique_id }}">
                <input type="hidden" name="message_target" value="team2">
                <input type="hidden" name="message_load" value="initial">
            </li>
            @endif
            @if(auth()->user()->id==1 || auth()->user()->id==2 || auth()->user()->id==15 || auth()->user()->id==24 || auth()->user()->id==34)
            <!-- team 3 -->
            <li id="{{ messagesUniqueID(auth()->user()->unique_id, 'team3') }}">
                <a href="javascript:void(0)" class="ajax-request messages-menu-link" data-type="form"
                    id="messages_team3_link"
                    data-form-id="{{ messagesUniqueID(auth()->user()->unique_id, 'team3') }}"
                    data-ajax-type="post" data-loading-target="chat-messages-container"
                    data-message-target="team3" data-progress-bar="hidden"
                    data-counter-id="messages_counter_team" data-url="{{ url('/messages/feed?action=load') }}">
                    <img src="{{ url('public/images/team.png') }}" alt="user-img" class="img-circle">
                    <span>Website Team
                        <small class="text-info messages_user_status text-lc">Keissiant, Brian, Moncho and Jim
                            <!--counter-->
                            <span class="badge badge-pill badge-danger messages_counter hidden"
                                id="messages_counter_team3">
                                <!--dynamic--></span>
                        </small>
                    </span>
                </a>
                <!--(dynamic)-->
                <input type="hidden" name="message_source" value="{{ auth()->user()->unique_id }}">
                <input type="hidden" name="message_target" value="team3">
                <input type="hidden" name="message_load" value="initial">
            </li>
            @endif
            <!-- users list-->
            ...
            <li id="{{ messagesUniqueID(auth()->user()->unique_id, $user->unique_id) }}">
                <a href="javascript:void(0)" class="ajax-request messages-menu-link" data-type="form"
                    ...
                    data-ajax-type="post" data-loading-target="chat-messages-container"
                    data-progress-bar="hidden" id="id_{{messagesCounterUniqueID($user->unique_id)}}" data-message-target="{{ $user->unique_id }}"
