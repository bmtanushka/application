...
function NXBootCards() {
    ...
    //CARD JOB NUMBER
    if ($("#cardModal").length) {
        NX.card_job_number_edit_original = '';
        //start
        $(document).on('click', '#card-job-number-editable', function () {
            NX.card_job_number_edit_original = $(this).html();
            $(this).hide();
            $(".card-job-number-input").val(NX.card_job_number_edit_original);
            $("#card-job-number-edit").show();
        });
        //cancel
        $(document).on('click', '#card-job-number-button-cancel', function () {
            $("#card-job-number-editable").html(NX.card_job_number_edit_original);
            $("#card-job-number-edit").hide();
            $("#card-job-number-editable").show();
        });
        //show loading annimation
        $(document).on('click', '#card-job-number-button-save', function () {
            $("#card-job-number-editable").html('<div class="loading w-px-150 h-px-30"></div>');
            $("#card-job-number-edit").hide();
            $("#card-job-number-editable").show();
            nxAjaxUxRequest($(this));
        });
     ...
     *--------------------------------------------*/
    if ($("#cardModal").length) {
        $('.file-comment-tinmyce').each(function() {
            ...
            $("#card_fileupload").dropzone({
                ...
                init: function () {
                    ...
                    this.on("sending", function (file, xhr, formData) {
                        ...
                        var completed = $("#completed").is(":checked"); // Get completed files check box value
